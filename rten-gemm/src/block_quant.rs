    ...
    pub fn batched_gemm_uninit<'a>(
        ...
        for (b, out_mat) in out.chunks_mut(n * m).enumerate() {
            ...
            for (row, out_row) in out_mat.chunks_mut(n).enumerate() {
                ...
                // Per-block sums of the LHS row, needed to correct the output
                // if the RHS has non-default zero points. See
                // `zero_point_correction`.
                let lhs_block_sums: Vec<f32> = if rhs.has_zero_points() {
                    let block_size = rhs.elements_per_block();
                    match &lhs_row {
                        LhsRow::Float(row) => row
                            .chunks_exact(block_size)
                            .map(|block| block.iter().sum())
                            .collect(),
                        LhsRow::Quant { data, scales } => data
                            .data()
                            .chunks_exact(block_size)
                            .zip(scales.iter())
                            .map(|(block, &scale)| {
...
fn zero_point_correction<I: Isa>(
    ...
    let n_blocks = col_scales.len().min(lhs_block_sums.len());
