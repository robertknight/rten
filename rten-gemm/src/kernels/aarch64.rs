...
unsafe impl Kernel<u8, i8, i32> for ArmInt8MMKernel {
    ...
    unsafe fn kernel(
        ...
        const NR: usize = ArmInt8MMKernel::NR;
        const NR_REGS: usize = ArmInt8MMKernel::NR / size_of::<i32>();

        // Each i8mm instruction accumulates into a 2x2 accumulator in a single
        // register. The output is 8x8 so we have 4x4 tiles.
        ...
        let a_tile_size = COL_TILES * u8_ops.len();
        let b_tile_size = ROW_TILES * u8_ops.len();
