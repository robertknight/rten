...
simd_type!(BF16x8, bf16, LEN_X32 * 2);
...
unsafe impl BitOps<bf16> for GenericIsa {
    type Simd = BF16x8;

    bit_ops_common!(BF16x8, bf16, 8, M16);

    #[inline]
    fn and(self, x: BF16x8, y: BF16x8) -> BF16x8 {
        x.map_with(y, |x, y| bf16::from_bits(x.to_bits() & y.to_bits()))
    }

    #[inline]
    fn not(self, x: BF16x8) -> BF16x8 {
        x.map(|x| bf16::from_bits(!x.to_bits()))
    }

    #[inline]
    fn or(self, x: BF16x8, y: BF16x8) -> BF16x8 {
        x.map_with(y, |x, y| bf16::from_bits(x.to_bits() | y.to_bits()))
    }

    #[inline]
    fn xor(self, x: BF16x8, y: BF16x8) -> BF16x8 {
        x.map_with(y, |x, y| bf16::from_bits(x.to_bits() ^ y.to_bits()))
    }
}

impl Extend<bf16> for GenericIsa {
    type Output = F32x4;
