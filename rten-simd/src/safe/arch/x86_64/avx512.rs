use std::arch::x86_64::{
    ...
    _mm512_add_epi8, _mm512_add_ps, _mm512_andnot_ps, _mm512_cmp_epi16_mask, _mm512_cmp_epi32_mask,
    _mm512_cmp_ps_mask, _mm512_cmpeq_epi8_mask, _mm512_cmpge_epi8_mask, _mm512_cmpgt_epi8_mask,
    _mm512_cvtepi8_epi16, _mm512_cvtps_epi32, _mm512_cvttps_epi32, _mm512_div_ps,
    ...
    _mm512_mullo_epi32, _mm512_packs_epi16, _mm512_reduce_add_ps, _mm512_set1_epi16,
...
unsafe impl SimdOps<I8x64> for Avx512Isa {
    ...
    fn mul(self, x: I8x64, y: I8x64) -> I8x64 {
        unsafe {
            // Extract low and high halves and sign-extend to i16.
            let x_lo = _mm512_extracti64x4_epi64(x.0, 0);
            let x_hi = _mm512_extracti64x4_epi64(x.0, 1);
            let y_lo = _mm512_extracti64x4_epi64(y.0, 0);
            let y_hi = _mm512_extracti64x4_epi64(y.0, 1);

            let x_lo = _mm512_cvtepi8_epi16(x_lo);
            let x_hi = _mm512_cvtepi8_epi16(x_hi);
            let y_lo = _mm512_cvtepi8_epi16(y_lo);
            let y_hi = _mm512_cvtepi8_epi16(y_hi);

            // Multiply in i16 and re-pack back to i8.
            let prod_lo = _mm512_mullo_epi16(x_lo, y_lo);
            let prod_hi = _mm512_mullo_epi16(x_hi, y_hi);

            _mm512_packs_epi16(prod_lo, prod_hi)
        }
        .into()
    }
...
impl SimdIntOps<I8x64> for Avx512Isa {
    ...
    fn shift_left<const SHIFT: i32>(self, x: I8x64) -> I8x64 {
        unsafe {
            ...
            let count = _mm512_set1_epi16(SHIFT as i16);

            let y_lo = _mm512_sllv_epi16(x_lo, count);
            let y_hi = _mm512_sllv_epi16(x_hi, count);

            _mm512_packs_epi16(y_lo, y_hi)
        }
        .into()
    }
}
