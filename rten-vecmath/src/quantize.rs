...
use rten_simd::safe::{Isa, Narrow, Simd, SimdFloatOps, SimdOp, SimdOps};
...
impl<'d> SimdOp for Quantize<'_, 'd, u8> {
    ...
    fn eval<I: Isa>(self, isa: I) -> Self::Output {
        ...
        let mut dest_chunks = self.dest.chunks_exact_mut(f32_v_len * 4);

        for (src_chunk, dest_chunk) in src_chunks.by_ref().zip(&mut dest_chunks) {
            let src: [I::F32; 4] =
                std::array::from_fn(|i| src_ops.load(&src_chunk[i * f32_v_len..][..f32_v_len]));
            let quant_i32: [I::I32; 4] = src.map(|x| {
                let y = src_ops.mul(x, scale_vec);
                let y = src_ops.to_int_round(y);
                let y = i32_ops.add(y, zp_vec);
                i32_ops.clamp(y, i32_ops.zero(), i32_ops.splat(u8::MAX as i32))
            });
            let quant_i16_low = i32_ops.narrow_truncate(quant_i32[0], quant_i32[1]);
            let quant_i16_high = i32_ops.narrow_truncate(quant_i32[2], quant_i32[3]);
            let quant_u16_low = quant_i16_low.reinterpret_cast::<I::U16>();
            let quant_u16_high = quant_i16_high.reinterpret_cast::<I::U16>();
            let quant_u8 = isa.u16().narrow_truncate(quant_u16_low, quant_u16_high);
            isa.u8().store_uninit(quant_u8, dest_chunk);
        }

        // Quantize tail elements.
        for (src, dest) in src_chunks
            .remainder()
            .iter()
            .zip(dest_chunks.into_remainder().iter_mut())
        {
        ...
        unsafe { transmute::<&mut [MaybeUninit<u8>], &mut [u8]>(self.dest) }
    }
}
