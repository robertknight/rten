//! Values with symbolic shapes and elements.
...
/// Value with symbolic shape and elements.
///
/// This is either a tensor where the elements and dimension sizes can be
/// concrete values or symbolic expressions, or a sequence of such tensors. This
/// type is used during shape inference to represent operator inputs and
/// outputs, as well as the values of operations which manipulate shapes.
///
...
///
/// Methods which describe a tensor's shape or values, such as
/// [`ndim`](Self::ndim) and [`values`](Self::values), return `None` for
/// sequences. Use [`as_sequence`](Self::as_sequence) to get a sequence's items.
///
/// ```
...
impl SymValue {
    /// Create a new value with an unknown shape and elements.
    ///
    /// This is also used for sequences whose length is unknown.
    ///
    /// `note` is a short string indicating the reason why the value is
    /// unknown. This is used for debugging purposes.
    ...
    /// Create a new symbolic sequence with a known length.
    pub fn from_sequence(items: Vec<SymValue>) -> Self {
        Self(SymValueKind::Sequence(items))
    }
    ...
    /// Return this value's items, if it is a sequence with a known length.
    pub fn as_sequence(&self) -> Option<&[SymValue]> {
        match &self.0 {
            SymValueKind::Sequence(items) => Some(items),
            _ => None,
        }
    }
    ...
    pub fn to_constant(&self) -> Option<Constant> {
        match &self.0 {
            ...
            SymValueKind::Shape(_) | SymValueKind::Sequence(_) | SymValueKind::Unknown { .. } => {
                None
            }
        }
    }

    /// Return the number of dimensions, if this value is a tensor with a
    /// known rank.
    pub fn ndim(&self) -> Option<usize> {
        match &self.0 {
            ...
            SymValueKind::Sequence(_) | SymValueKind::Unknown { .. } => None,
        }
    }
    ...
    /// Return true if this value's shape is not fully known.
    ///
    /// This is the case if the rank of a tensor is unknown, or the size of any
    /// dimension is a symbol that was generated by [`SymbolGen`] to stand in
    /// for a size that could not be computed. For a sequence this is true if
    /// the length is unknown or the shape of any item is not fully known.
    pub fn has_unknown_shape(&self) -> bool {
        if let SymValueKind::Sequence(items) = &self.0 {
            return items.iter().any(|item| item.has_unknown_shape());
        }
        let Some(mut dims) = self.shape() else {
            return true;
        };
        dims.any(|dim| {
            dim.iter().any(|expr| match expr {
    ...
    pub fn simplify(self) -> Self {
        match self.0 {
            ...
            SymValueKind::Sequence(items) => {
                Self::from_sequence(items.into_iter().map(|item| item.simplify()).collect())
            }
            _ => self,
        }
    }
    ...
    pub fn replace_complex_expressions(&mut self, depth: u32, sym: &mut SymbolGen) -> bool {
        fn replace_complex(expr: &mut SymExpr, depth: u32, sym: &mut SymbolGen) -> bool {
        ...
        match &mut self.0 {
            SymValueKind::Scalar(item) => replace_complex(item, depth, sym),
            SymValueKind::Vector(vals) | SymValueKind::Shape(vals) => {
                vals.iter_mut().fold(false, |had_complex, expr| {
                    replace_complex(expr, depth, sym) | had_complex
                })
            }
            SymValueKind::Sequence(items) => items.iter_mut().fold(false, |had_complex, item| {
                item.replace_complex_expressions(depth, sym) | had_complex
            }),
            SymValueKind::Unknown { .. } => false,
        }
    }
