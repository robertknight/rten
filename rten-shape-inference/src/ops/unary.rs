...
impl InferShapes for Neg {
    fn infer_shapes(
        ...
        if let Some(item) = data.as_scalar() {
            return Ok([SymValue::from_scalar(-item.clone())].into());
        } else if let Some(vec) = data.as_vector() {
            let neg_vec = vec.iter().map(|item| -item.clone()).collect();
            return Ok([SymValue::from_vec(neg_vec)].into());
        }
...
mod tests {
    ...
    fn test_neg() {
        ...
        // Symbolic scalar
        let x = SymValue::from_scalar("batch".into());
        let result = Neg.infer_shapes([x].into(), &mut sym_gen).unwrap();
        assert_eq!(result[0], SymValue::from_scalar(-SymExpr::from("batch")));
        ...
        assert_eq!(
            result[0],
            SymValue::from_vec(vec![-SymExpr::from("batch"), -SymExpr::from(64),])
        );
    }
}
