...
/// An Encoder handles the process of converting individual sequences of
/// characters into a sequence of integer token IDs.
pub trait Encoder {
    /// Map the text version of a token to its numeric ID. This is used for
    /// looking up the ID for special tokens such as sequence separators.
    fn get_token_id(&self, token: &str) -> Result<usize, TokenizerError>;

    /// Map a token ID to its canonical string.
...
mod tests {
    ...
    // The tests below use the WordPiece encoder to exercise Tokenizer. This
    // is convenient as WordPiece is a pretty simple encoder. In principle it
    // should be possible to swap in other encoders.
