...
fn mel_filters(n_mels: u32, n_fft: u32, sample_rate: u32) -> NdTensor<f32, 2> {
    let min_hz = 0;
    let max_hz = sample_rate / 2;

    let mut filters = NdTensor::zeros([n_mels as usize, (n_fft / 2 + 1) as usize]);
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let mut mel_spec =
        log_mel_spectrogram(&audio, samples_per_chunk, n_mels, sample_rate).into_dyn();
