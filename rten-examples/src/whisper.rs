...
/// Return a matrix that maps a power spectrum with frequencies in Hz to one in
/// mels.
///
/// The returned matrix has shape `[n_mels, n_fft / 2 + 1]`.
fn mel_filters(n_mels: u32, n_fft: u32) -> NdTensor<f32, 2> {
    // TODO - Convert mel scale to hertz
    // TODO - Compute mel filters properly
    let vals: Vec<f32> = read_json_file("mel80.json")
        .as_array()
        .unwrap()
        .iter()
        .map(|x| x.as_f64().unwrap() as f32)
        .collect();
    NdTensor::from_data([n_mels as usize, (n_fft / 2 + 1) as usize], vals)
}
...
fn log_mel_spectrogram(audio: &[f32], padded_len: usize, n_mels: u32) -> NdTensor<f32, 2> {
    let hop_length = 160;
    let n_fft = 400;
    ...
    let mel_filters = mel_filters(n_mels, n_fft as u32);
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let mut mel_spec = log_mel_spectrogram(&audio, samples_per_chunk, n_mels).into_dyn();
