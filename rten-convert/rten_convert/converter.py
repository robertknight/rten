...
def build_constant_node(
    ...
    store_inline = tensor_data is None

    if store_inline:
        inline_data_vec = builder.CreateNumpyVector(constant.data.flatten())
    else:
        # When data is stored separately, we write an empty array in the model
        # for backwards compatibility.
        empty_array = np.array([], dtype=np.float32)
        inline_data_vec = builder.CreateNumpyVector(empty_array)
    ...
    # Add inline data. This is an empty placeholder if the data is stored
    # externally.
    sg.ConstantNodeAddDataType(builder, inline_data_type)
    sg.ConstantNodeAddData(builder, inline_data)

    # Add reference to separately stored data.
    if tensor_data and not store_inline:
        offset = tensor_data.add_tensor(constant.data)
        sg.ConstantNodeAddDataOffset(builder, offset)
...
def main():
    ...
    with open(output_path, "wb") as output:
        if use_v2_format:
            header_size = 32
            model_data_align = 4
            model_data_offset = round_up(header_size, model_data_align)
            ...
            write_padding(output, model_data_offset - output.tell())
