...
use wasnn::{Dimension, Input, Model, NodeId, Output, RunOptions};
use wasnn_tensor::prelude::*;
use wasnn_tensor::{RandomSource, Tensor};
...
Usage: {bin_name} [OPTIONS] <model>
  ...
  -h, --help     Print help
",
                    bin_name = parser.bin_name().unwrap_or("wasnn")
                );
                std::process::exit(0);
            }
...
/// cargo run -p wasnn-cli --release output.model
/// ```
///
/// To get detailed timing information set the `WASNN_TIMING` env var before
