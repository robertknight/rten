...
impl InferResult {
    #[cfg(test)]
    fn get_constant(&self, node: NodeId) -> Option<&Constant> {
        self.shapes.get(&node).and_then(|shape| match shape {
            Shape::Constant { index } => Some(&self.constants[*index]),
            Shape::Shape(_) => None,
        })
    }
}
...
/// Performs incremental shape and type inference for values in a graph.
///
/// This is used by constructing an instance using [`new`](Self::new), then
/// calling [`infer`](Self::infer) for each operator in the graph in
/// toplogical order. Finally [`finish`](Self::finish) should be called to
/// get the results.
///
/// In a model with subgraphs, shape inference for the parent graph is
/// interleaved with inference for subgraphs. Shape inference is run in the
/// parent graph until an operator is encountered with a subgraph. Inference is
/// then performed for the subgraph and when complete, inference continues for
/// operators in the parent graph. It is necessary to perform inference for
/// operators in the parent graph that are before the subgraph operator first,
/// so that the shapes and types of values captured by the subgraph are
/// available when inference is run in the subgraph.
...
impl InferredShapes {
    ...
    /// Finish shape and type inference and return the inferred shapes and
    /// types.
...
fn add_typed_constant(
    ...
    where
        graph::Constant: From<graph::ConstantNode<T>>,
    {
...
pub fn apply_shapes(graph: &mut Graph, shapes: InferResult) {
    // Map of `(const_index, value_dtype) => const_id` for constant nodes
    // created from shape inference constants. This exists because a single
    // constant from shape inference might need to be converted into multiple
    // constant nodes with different dtypes.
    let mut const_ids: HashMap<(usize, DataType), NodeId> = HashMap::new();

    let replace_value = |graph: &mut Graph, old_value_id, new_value_id| {
        // Replace `old_value_id` in operator inputs.
        if let Some(consumer_ids) = graph.get_consumers(old_value_id) {
        ...
        // Replace `old_value_id` in operator outputs.
        if let Some(out_idx) = graph.output_ids().iter().position(|id| *id == old_value_id) {
            let mut outs = graph.output_ids().to_vec();
            outs[out_idx] = new_value_id;
            graph.set_output_ids(&outs);
        }
    };
        ...
        match shape {
            Shape::Constant { index } => {
                let dtype = graph
                    ...
                    .or_else(|| shapes.types.get(&value_id).copied());
                let Some(ValueType::Tensor(dtype)) = dtype else {
                    // Without a known element type we can't produce a correctly
                    // typed constant, so leave the value as is.
                    continue;
                };
                ...
                let const_id = match const_ids.entry((index, dtype)) {
                    ...
                    Entry::Vacant(entry) => {
                        let Some(const_id) =
                            add_typed_constant(graph, &shapes.constants[index], dtype)
                        else {
...
mod tests {
    ...
    use crate::ops::{
        Add, Concat, Gather, Gemm, Identity, MatMul, Shape as ShapeOp, Split, Unsqueeze,
    };
    use crate::value::{DataType, ValueType};
    ...
    fn test_apply_shapes_replaces_values_with_constants() {
        ...
        let output_id = graph.output_ids()[0];
        let shapes = infer_shapes(&graph, Default::default()).unwrap();
        assert_eq!(shapes.get_constant(output_id), Some(&Constant::Scalar(64)));
        apply_shapes(&mut graph, shapes);

        // The output should be replaced with a constant as it doesn't depend on
        // model inputs.
        let new_output_id = graph.output_ids()[0];
        assert_ne!(new_output_id, output_id);
        let output = graph.get_node(new_output_id).unwrap();
        assert_eq!(output.as_constant().and_then(|c| c.as_scalar()), Some(64));
    }

    #[test]
    fn test_apply_shapes_constants_preserve_value_type() {
        let mut graph = {
            let x = Expr::value_with_info(
                "data",
                ValueType::Tensor(DataType::Float),
    ...
    #[test]
    fn test_apply_shapes_constants_shared_by_values_with_different_types() {
