        ...
        if let Some(data_offset) = constant.data_offset() {
            ...
            let graph_node = match constant.dtype() {
                sg::ConstantDataType::Int32 => {
                    let const_data =
                        constant_node_from_storage_offset::<i32>(storage, &shape, data_offset)?;
                    graph.add_constant(name, const_data)
                }
                sg::ConstantDataType::Float32 => {
                    let const_data =
                        constant_node_from_storage_offset::<f32>(storage, &shape, data_offset)?;
                    graph.add_constant(name, const_data)
                }
                _ => {
                    return Err(ModelLoadError::GraphError(
...
/// Create a graph constant node from data stored in the tensor data segment
/// of a model file.
///
/// `tensor_data_offset` specfies the offset of the start of the data from the
/// start of the storage.
fn constant_node_from_storage_offset<T: Pod>(
    storage: &Arc<ConstantStorage>,
    shape: &[usize],
    ...
    let Some(bytes) = storage
        .data()
        .get(tensor_data_offset..tensor_data_offset + byte_len)
    else {
        return Err(ModelLoadError::GraphError(
            "invalid tensor data offsets".to_string(),
        ));
    };
    ...
    if bytes.as_ptr() as usize % std::mem::align_of::<T>() == 0 {
        ...
        Err(ModelLoadError::GraphError(
            "storage is not correctly aligned".to_string(),
        ))
    }
}
