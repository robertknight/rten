...
use crate::slice_reductions::slice_fold_assoc;
...
pub fn hardmax(pool: &BufferPool, input: TensorView, axis: isize) -> Result<Tensor, OpError> {
    ...
    normalize_lanes(&mut output, axis, |lane| {
        ...
        let max_val = slice_fold_assoc(lane, f32::NEG_INFINITY, |acc, x| acc.max(x));
...
mod tests {
    ...
    use super::{
        LpNormalization, NanHandling, batch_norm, batch_norm_in_place, hardmax,
        instance_normalization, layer_normalization, log_softmax, lp_normalization,
