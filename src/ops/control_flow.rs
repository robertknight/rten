...
impl SubgraphOperator for Scan {
    ...
    fn run_subgraph<'a>(
        ...
        for i in 0..self.num_scan_inputs {
            ...
            match sequence_len {
                Some(expected) if expected != len => {
                    return Err(make_run_error(OpError::IncompatibleInputShapes(
        ...
        if sequence_len == 0 && n_scan_outputs > 0 {
            return Err(make_run_error(OpError::UnsupportedValue(
    ...
    Tensor<T>: TryFrom<Value>,
{
    fn append_element<T: Copy>(
        ...
        if element.ndim() + 1 != output.ndim() {
            return Err(OpError::InvalidValue(SHAPE_MISMATCH));
        }
        element.insert_axis(axis);
        output
            .append(axis, &element)
            .map_err(|_| OpError::InvalidValue(SHAPE_MISMATCH))?;
