...
fn load_graph(
    ...
    for onnx_op in &onnx_graph.node {
        ...
        // Perform shape and type inference for node. This assumes that nodes
        // are loaded in topological order.
    ...
    // Update value nodes with inferred shape and type information.
    if let Some(shapes) = shapes {
        apply_shapes(&mut graph, shapes.finish());
    }
