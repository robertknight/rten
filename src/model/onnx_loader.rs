...
pub fn load(
    ...
    let reader = ValueReader::new(ReadPos::new(BufReader::new(file)));
    let model =
        ModelProto::decode(reader).map_err(|err| ModelLoadError::ParseFailed(Box::new(err)))?;
    ...
    let constant: Constant = match initializer.data_type {
        Some(onnx::DataType::FLOAT) => {
            if let Some(data) = raw_data {
                let data = ArcSlice::<f32>::from_bytes(data).unwrap();
                // TODO - Catch errors if size doesn't match.
                let tensor = ArcTensorView::from_data(&shape, data);
                Constant::new(name, tensor)
            } else if let Some(loc) = external_location {
                let u32s = external_loader.load_u32(loc)?;
                let data = cast_pod_vec(u32s).unwrap();
                let tensor = load_tensor::<f32>(&shape, data, name)?;
                Constant::new(name, tensor.into_arc())
            } else {
                let data = initializer.float_data.as_slice().into();
