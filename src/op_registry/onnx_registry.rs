...
impl OnnxOpRegistry {
    ...
    pub(crate) fn read_op(&self, op: &onnx::NodeProto, ctx: &dyn OpLoadContext) -> ReadOpResult {
        let op_type = op.op_type.unwrap_or_default();
        self.ops
            .get(op_type)
            .ok_or_else(|| ReadOpError::OperatorUnavailable {
                // TODO - Make the name field non-static so we can convert `op_type` into
                // a string.
                name: None,
            })
...
struct Attrs<'a> {
    attrs: &'a [onnx::AttributeProto<'a>],
    used_attrs: RefCell<SmallVec<[&'static str; 6]>>,
}

impl<'a> Attrs<'a> {
    fn new(attrs: &'a [onnx::AttributeProto<'a>]) -> Self {
        Self {
            attrs,
    ...
    fn get(&self, name: &'static str) -> Option<Attr<'a>> {
        ...
        let val = self.attrs.iter().find(|att| att.name == Some(name))?;
    ...
    fn require(&self, name: &'static str) -> Result<Attr<'a>, ReadOpError> {
        self.get(name).ok_or_else(|| ReadOpError::AttrError {
            attr: name,
...
struct Attr<'a> {
    name: &'static str,
    attr: &'a onnx::AttributeProto<'a>,
}

impl<'a> Attr<'a> {
    fn new(name: &'static str, attr: &'a onnx::AttributeProto<'a>) -> Self {
        Self { name, attr }
    }
    ...
    fn as_graph(&self) -> Result<&onnx::GraphProto<'a>, ReadOpError> {
        self.attr.g.as_ref().ok_or_else(|| ReadOpError::AttrError {
            attr: self.name,
            error: "attribute is not a graph",
        })
    }
    ...
    fn as_string_enum<T>(&self, map: impl Fn(&str) -> Option<T>) -> Result<T, ReadOpError> {
        ...
        map(str_val).ok_or_else(|| ReadOpError::AttrError {
            attr: self.name,
        ...
        .map(|attr| {
            let Some(tensor) = attr.attr.t.as_ref() else {
                return Err(ReadOpError::AttrError {
                    attr: "value",
                    error: "missing tensor value",
                });
            };

            match tensor.data_type {
                Some(onnx::DataType::FLOAT) => {
                    let value = extract_scalar(tensor.raw_data, &tensor.float_data)?;
                    Ok(Scalar::Float(value))
                }
                Some(onnx::DataType::INT64) => {
                    let value: i64 = extract_scalar(tensor.raw_data, &tensor.int64_data)?;
                    Ok(Scalar::Int(value as i32))
                }
                _ => Err(ReadOpError::AttrError {
                    attr: "value",
                    error: "unsupported data type for ConstantOfShape",
                }),
            }
        })
        .transpose()?
...
impl_read_op!(ConvTranspose, |attrs: &Attrs| {
    let groups = attrs.get_as("group").unwrap_or(1);
    let groups = groups as usize; // TODO - Handle invalid values
