    ...
    fn propagate_constants(&self, graph: &mut GraphMutator) -> Result<(), OptimizeError> {
        ...
        for (value_node_id, output) in leaves {
            ...
            let const_id = match output {
                Value::FloatTensor(tensor) => Some(graph.add_constant(const_name, tensor)),
                Value::Int32Tensor(tensor) => Some(graph.add_constant(const_name, tensor)),
                Value::Int8Tensor(tensor) => Some(graph.add_constant(const_name, tensor)),
                Value::UInt8Tensor(tensor) => Some(graph.add_constant(const_name, tensor)),
